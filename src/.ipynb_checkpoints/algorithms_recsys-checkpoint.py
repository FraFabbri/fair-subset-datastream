...
def reservoir_sampling(array, k):
    ...
    if size_left < 0:
        ...
        #print(len(array), size_left, len(left_array))    
...
class Algorithms():
    ...
    def __init__(self, data, tracking_filename):
        ...
        self.S = set()
        self.f_S = 0
        self.S_neigh = np.zeros(len(data.mapping_id_to_attributes))
    ...
    def runFairgreedy(self, data, func, func_delta):        
        ...
        t0 = time.time()
        self.track[track_count] = (self.f_S, 0)
        self.save_tracking()
        ...
        while self.count_k_i != self.output_partition:
            ...
            # tracking the improvement
            track_count = len(self.track)
            self.track[track_count] = (self.f_S, time.time() - t0)
            self.save_tracking()
    ...
    def randomBaseline(self, data, func):
        ...
        # tracking the improvement
        track_count = len(self.track)
        self.track[track_count] = (f_avg, time.time() - t0)
        self.save_tracking()
    ...
    def fairOnepass(self, data, func, func_delta, alpha, beta, option="no-buffer", factor=2):
        ...
        for v in data.mapping_id_to_attributes:
            ...
            while selected_j > self.j_min:
                ...
                if updated_1 == True:
                    ...
                    self.history_f_S_tau[max_tau], self.S_neigh_tau[max_tau] = func(data, self.all_S_tau[max_tau])
                    ...
                    for remaining_tau in self.all_S_tau:
                        if remaining_tau < max_tau:
                            ...
                            if count_ki_single_tau < k_i:
                                ...
                                self.history_f_S_tau[remaining_tau], self.S_neigh_tau[remaining_tau] = func(data, self.all_S_tau[max_tau])
        ...
        if option != "no-buffer":
            ...
            if option == "buffer-size":
                B_fv = []
                for i in self.B:
                    B_fv_group_i = [(v_buffer, self.history_best_fv[v_buffer], i) for v_buffer in self.B[i]]
                    B_fv += B_fv_group_i

                B_fv = sorted(B_fv, key=lambda x: x[1], reverse=True)
                B_fv = B_fv[:self.B_limit]
                for i in self.B:
                    self.B[i] = intbitset([v for v, value, ii in B_fv if ii == i])
